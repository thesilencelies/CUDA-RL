...
class MancalaEnv():
  ...
  def take_turn(self, action: int, player: int) -> bool:
    ...
    self.turn_record.append((action, player))
   # print(f"move {action}")
    ...
    if self._pots[index] == 1 and \
        index >= player*(self._npots + 1) and \
         index < pool_index:
      opp_index = (self._npots * 2 - index) % len(self._pots)
    ...
    for i in range(self._npots + 1, self._npots*2 + 1):
      if self._pots[i] > 0:
        player2empty = False
        break;
  ...
  def get_pots(self, player: int) -> [int]:
